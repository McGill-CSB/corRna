...
def getAvg():
	...
	print "<Under Seq> :: under "+str(nAVG)+" on A84_U86G or user input threshold"
...
def loop(node):	
	...
	for i in range(int(LOOP_SIZE)):
		...
		node = NODE
		start = time()
		out = commands.getoutput('./RNAmutants -l lib -m '+str(MUTATION_DEPTH) +' -n '+str(NSIZE) +' -f wrapper.in -C')
		end = time() - start
		totalTIME+=end
	#	node = lockS(node)
		for line in out.splitlines():
	...
	print BUCKETS	
	print 'No of SIGS under Seq threshold {0} = {1}'.format(nAVG,len(SIGNIFICANT))
