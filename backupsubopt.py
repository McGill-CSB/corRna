...
def printSeq(struct, boot):
	EMAILMSG = "<table><thead><td>Sequence</td><td>Structure</td><td>Correlation</td><td>MFE</td><td>BreakNumber Set</td><td>Structure Set</td></thead><tbody>"
	json = '[{"method":"structural","wild":"'+SEQUENCE+'","struct":"'+struct+'","seq":['
	for i in range(len(boot)):
		if(DEBUG):
			print boot[i]
		if JSON:
			json+= '{"seq":"'+str(boot[i][0])+'", "struct":"'+str(boot[i][1])+'", "corr":"'+str(boot[i][2])+'", "mfe":"'+str(boot[i][3])+'", "breakNumber":"'+str(boot[i][4])+'", "breakStruct":"'+str(boot[i][5])+'"}'
			if i != len(boot)-1:
				json+=','
			
		if EMAIL:
			EMAILMSG += "<tr><td>"+str(boot[i][0])+"</td><td>"+str(boot[i][1])+"</td><td>"+str(boot[i][2])+"</td><td>"+str(boot[i][3])+"</td><td>"+str(boot[i][4])+"</td><td>"+str(boot[i][5])+"</td></tr>"
	if JSON:
		json+=']}]'
		print json
	if EMAIL:
...
if __name__ == "__main__":
	try: 
		optlist, args = getopt.getopt(sys.argv[1:], 'khs:xda:m:n:b:ju:', ["help"])
